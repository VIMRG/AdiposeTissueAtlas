...
Heatmap_fun <- function(seurat_object, subset = F, R, metadata = data_hatim, cluster, genes, tmp_dir = paste0("/data/p_koethe_lab/Atlas_AT/HATIM_Analysis/", date, "/Dialogue/", "All"),
    ...
    row_ha = rowAnnotation(Group = merged$StudyGroup, Sex = merged$Sex, BMI = merged$BMI, Age = merged$Age, FBG = merged$FBG, HbA1c = merged$hba1c, col = list(Group = c("HIV+ non-diabetic"="green", "HIV+ prediabetic" = "blue", "HIV+ diabetic" = "orange", "HIV- diabetic" = "red"),
...
tmp_dir <- paste0("/data/p_koethe_lab/Atlas_AT/HATIM_Analysis/", date, "/Dialogue/", "HIVneg")
dir.create(tmp_dir)
...
# 5. PLOT HEATMAP_____________________________________________________
R <- readRDS(paste(tmp_dir, "Results_HIVneg.rds", sep = "/"))


###############################
# Generate Heatmap Overall
###############################
# CD4 MCP1
plot <- Heatmap_fun(seurat_object = CD4, subset = F, metadata = data_hatim, cluster = "CD4", genes = c("CCR7", "LDHB", "LEF1", "LEPROTL1", "WNT7A", "TCF7", "SELL", "ALOX5AP", "CCL5", "CD36",
"ANXA1", "ANXA2", "CCL4L2", "APOBEC3G", "CTSD", "COTL1", "FOS", "FABP4", "GBP5", "GZMK", "GZMA", "HOPX", "IFITM3", "IFNG", "IL32", "KLRG1", "LGALS3", "S100A4", "SH3BGRL3", "TNF", "VIM"), 
tmp_dir = fig_dir, filename = "Figure8E.png", R = R, MCP_dim = 1)
