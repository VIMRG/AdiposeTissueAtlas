...
Doublet <- readRDS("../Initial_HIVnegative.rds")

#3. REMOVE DOUBLETS_____________________________________________________
###############################
# Doublet Removal (this should mostly be HIV-negative)
###############################
...
# Clusters 94,93,91,90,81,78,76,75,74,52,44,32,
###############################
# Tag Junk and Doublets
###############################
Singlet[['Junk']] <- ifelse(Singlet$seurat_clusters %in% c(36,60,63), "Junk", "Not Junk")
Singlet[['Heterotypic']] <- ifelse(Singlet$seurat_clusters %in% c(32,44,52,74,75,76,78,81,90,91,93,94), "Heterotypic", "Singlet")
DotPlot(Singlet, group.by = "Heterotypic", features = c("LYZ", "CD68", "CD3E", "NKG7", "CLDN5", "GNG11", "MS4A1", "JCHAIN", "CCDC80", "LUM", "LILRA4")) + RotatedAxis() + coord_flip()
...
saveRDS(Singlet, file = paste(tmp, "Singlet_Integrated_revised.RDS", sep = "/"))
...
Lymphoid <- subset(Singlet, idents = c(33,22,68,35,27,75,19,15,42,89,66,80)) #12
saveRDS(Lymphoid, file = paste(tmp, "Lymphoid_HIVneg.rds", sep = "/"))

Stromal <- subset(Singlet, idents = c(72,78,3,38,26,40,7,29,46,88,37,14,24,0,2,32,77, 4,6,13,64,1,47,91,63)) #25
saveRDS(Stromal, file = paste(tmp, "Stromal_HIVneg.rds", sep = "/"))

Myeloid <- subset(Singlet, idents = c(87,56,82,94,79,9,10,81,11,39,76,18,54,48,44,65,43,17,45,84,23,16,51,83,59,85,86,71)) #28 clusters
saveRDS(Myeloid, file = paste(tmp, "Myeloid_HIVneg.rds", sep = "/"))

Bcells <- subset(Singlet, idents = c(50,92)) #2
saveRDS(Bcells, file = paste(tmp, "Bcells_HIVneg.rds", sep = "/"))

Vascular <- subset(Singlet, idents = c(91,70,55,58,69,21,25,61,20,57,73,53,34,36,52,60,28,67,90,74,41,30,62,49,8,12,5,31)) # 28
