...
###-------------------####
# FBG Adjusted (PVM)
Cor_res <- Cor_rho(Macrophage.prop, adjusted = T, variable = "meta_fbg", nonDM = T)
FBG_Mac <- Cor_plot(Cor_res, Macrophage, title = "FBG")

# HbA1c Adjusted (LAM)
Cor_res <- Cor_rho(Macrophage.prop, adjusted = T, variable = "meta_hba1c", nonDM = T)
HbA1c_Mac <- Cor_plot(Cor_res, Macrophage, title = "HbA1c")
plot_grid(FBG_Mac, HbA1c_Mac)
...
###-------------------####
# FBG Adjusted (cMo, Other Mo, Cycling)
Cor_res <- Cor_rho(Myeloid.prop, adjusted = T, variable = "meta_fbg", nonDM = T)
FBG_Myeloid <- Cor_plot(Cor_res, Myeloid, title = "FBG")

# HbA1c Adjusted (nMo, Other Mo, IM, cMo, Mo-Mac, LAM)
Cor_res <- Cor_rho(Myeloid.prop, adjusted = T, variable = "meta_hba1c", nonDM = T)
HbA1c_Myeloid <- Cor_plot(Cor_res, Myeloid, title = "HbA1c")
plot_grid(FBG_Myeloid, HbA1c_Myeloid)
...
###-------------------####
# FBG Adjusted (CD8 TEM, CD8 Naive)
Cor_res <- Cor_rho(CD8.prop, adjusted = T, variable = "meta_fbg", nonDM = T)
FBG_CD8 <- Cor_plot(Cor_res, CD8, title = "FBG")

# HbA1c Adjusted ()
Cor_res <- Cor_rho(CD8.prop, adjusted = T, variable = "meta_hba1c", nonDM = T)
HbA1c_CD8 <- Cor_plot(Cor_res, CD8, title = "HbA1c")

plot_grid(FBG_CD8, HbA1c_CD8)

ggsave(file = paste(fig_dir, "Figure3E.png", sep = "/"), device = "png", dpi = 300, units = "in", height = 5, width = 10)
...
###-------------------####
# FBG Adjusted (CD4 TCM, CD4 Naive, CD4 TEM)
Cor_res <- Cor_rho(CD4.prop, adjusted = T, variable = "meta_fbg", nonDM = T)
FBG_CD4 <- Cor_plot(Cor_res, CD4, title = "FBG")

# HbA1c Adjusted ()
Cor_res <- Cor_rho(CD4.prop, adjusted = T, variable = "meta_hba1c", nonDM = T)
HbA1c_CD4 <- Cor_plot(Cor_res, CD4, title = "HbA1c")

plot_grid(FBG_CD4, HbA1c_CD4)

ggsave(file = paste(fig_dir, "Figure3G.png", sep = "/"), device = "png", dpi = 300, units = "in", height = 5, width = 10)
...
# Macrophage
Mac_DM <- Hmisc::summaryM(LAM + PVM + IM + `Mo-Mac` ~ StudyGroup, data = Macrophage.prop[Macrophage.prop$StudyGroup != "HIV+ prediabetic",], overall = TRUE, test = TRUE)
Mac_preDM <- Hmisc::summaryM(LAM + PVM + IM + `Mo-Mac` ~ StudyGroup, data = Macrophage.prop[Macrophage.prop$StudyGroup != "HIV+ diabetic",], overall = TRUE, test = TRUE)
Results_Mac <- FDR_Fun(df1 = Mac_DM, df2 = Mac_preDM, clusters = c("LAM", "IM", "PVM", "Mo-Mac"))
...
# Myeloid
Myeloid_DM <- Hmisc::summaryM(cMo + nMo + `ISG+ Mo` + `Other Mo` + `Mo-Mac` + LAM + IM + PVM + cDC1 + cDC2B + pDC + `Migratory DC` + `Cycling Myeloid` + `Other Mac`  ~ StudyGroup, data = Myeloid.prop[Myeloid.prop$StudyGroup != "HIV+ prediabetic",], overall = TRUE, test = TRUE)
Myeloid_preDM <- Hmisc::summaryM(cMo + nMo + `ISG+ Mo` + `Other Mo` + `Mo-Mac` + LAM + IM + PVM + cDC1 + cDC2B + pDC + `Migratory DC` + `Cycling Myeloid` + `Other Mac`  ~ StudyGroup, data = Myeloid.prop[Myeloid.prop$StudyGroup != "HIV+ diabetic",], overall = TRUE, test = TRUE)
Results_Myeloid <- FDR_Fun(df1 = Myeloid_DM, df2 = Myeloid_preDM, clusters = c("cMo", "nMo", "ISG+ Mo", "Other Mo", "Mo-Mac", "LAM", "PVM", "IM", "cDC1", "cDC2B", "pDC", "Migratory DC", "Cycling Myeloid", "Other Mac"))
...
`Mature Preadipocyte 2` + `Myofibroblast` + `ISG+ Preadipocyte` + `Metallothionein+ Preadipocyte` + `Cycling Myofibroblast` ~ StudyGroup, data = Stromal.prop[Stromal.prop$StudyGroup != "HIV+ prediabetic",], overall = TRUE, test = TRUE)
...
`Mature Preadipocyte 2` + `Myofibroblast` + `ISG+ Preadipocyte` + `Metallothionein+ Preadipocyte` + `Cycling Myofibroblast` ~ StudyGroup, data = Stromal.prop[Stromal.prop$StudyGroup != "HIV+ diabetic",], overall = TRUE, test = TRUE)
Results_Stromal <- FDR_Fun(df1 = Stromal_DM, df2 = Stromal_preDM, clusters = as.character(colnames(Stromal.prop)[c(1:12)]))
...
`CD57 mNK` + `CD16 mNK` + `Immature NK` + ILC + `Cyclcing T/NK` + MAIT + `Gamma Delta` ~ StudyGroup, data = Lymphoid.prop[Lymphoid.prop$StudyGroup != "HIV+ prediabetic",], overall = TRUE, test = TRUE)
...
`CD57 mNK` + `CD16 mNK` + `Immature NK` + ILC + `Cyclcing T/NK` + MAIT + `Gamma Delta` ~ StudyGroup, data = Lymphoid.prop[Lymphoid.prop$StudyGroup != "HIV+ diabetic",], overall = TRUE, test = TRUE)
...
# Vascular
Vascular_DM <- Hmisc::summaryM(`Intermediate Capillary EC` + `Capillary EC` + `Ven-Cap EC` + `Venous EC` + `Venous EndoMT-like` + `Cycling Vascular Cells` + `Arterial EndoMT-like` +
`Capillary EndoMT-like` + `Pericyte` + `VSMC` + `Arterial EC` ~ StudyGroup, data = Vascular.prop[Vascular.prop$StudyGroup != "HIV+ prediabetic",], overall = TRUE, test = TRUE)
Vascular_preDM <- Hmisc::summaryM(`Intermediate Capillary EC` + `Capillary EC` + `Ven-Cap EC` + `Venous EC` + `Venous EndoMT-like` + `Cycling Vascular Cells` + `Arterial EndoMT-like` +
`Capillary EndoMT-like` + `Pericyte` + `VSMC` + `Arterial EC` ~ StudyGroup, data = Vascular.prop[Vascular.prop$StudyGroup != "HIV+ diabetic",], overall = TRUE, test = TRUE)
Results_Vascular <- FDR_Fun(df1 = Vascular_DM, df2 = Vascular_preDM, clusters = as.character(colnames(Vascular.prop)[c(1:11)]))
