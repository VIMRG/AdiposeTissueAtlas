...
###-------------------####
Gene_Up <- c(R$MCPs$MCP1[[1]], R$MCPs$MCP1[[2]], R$MCPs$MCP1[[3]], R$MCPs$MCP1[[4]], R$MCPs$MCP1[[5]], R$MCPs$MCP1[[6]], R$MCPs$MCP1[[7]], R$MCPs$MCP1[[8]], R$MCPs$MCP1[[9]], R$MCPs$MCP1[[10]])
Gene_Down <- c(R$MCPs$MCP1[[11]], R$MCPs$MCP1[[12]], R$MCPs$MCP1[[13]], R$MCPs$MCP1[[14]], R$MCPs$MCP1[[15]], R$MCPs$MCP1[[16]], R$MCPs$MCP1[[17]], R$MCPs$MCP1[[18]], R$MCPs$MCP1[[19]])
...
Up_path <- enrich_Up[1:8, c("Description", "p.adjust", "GeneRatio")] # rna processes
Up_path$Type <- "Up"
Down_path <- enrich_Down[1:8, c("Description", "p.adjust", "GeneRatio")] # extracellular organization
Down_path$Type <- "Down"
...
plot <- pathways %>% group_by(Type) %>%
        ...
        axis.text.y = element_text(size = 18, face = "bold"), legend.title = element_text(size = 20, face = "bold"), legend.position = "top") + scale_y_discrete(labels = function(x) str_wrap(x, width = 50))
    
plot
ggsave(file = paste(fig_dir, "Figure7B.png", sep = "/"),  device = 'png', width = 10, height = 10, units = 'in', dpi = 600)
...
plot
ggsave(file = paste(fig_dir, "Figure7B_legend.png", sep = "/"),  device = 'png', width = 15, height = 10, units = 'in', dpi = 600)
...
"TNF", "XCL1", "XCL2"), tmp_dir = fig_dir, filename = "Figure7C.png", R = R, MCP_dim = 1, height = 10, width = 15)
...
filename = "Figure7D.png", tmp_dir = fig_dir, R = R, MCP_dim = 1, height = 10, width = 15)
