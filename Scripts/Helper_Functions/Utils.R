...
colors <- c("aquamarine1", "bisque", "blue", "burlywood", "chartreuse", "coral", "cyan", "chocolate", "darkolivegreen", "darkorange", "darksalmon", "deepskyblue", "dodgerblue",
"firebrick", "gold", "gray", "green", "greenyellow", "khaki", "lightcoral", "lightcyan", "lightgreen", "lightpink", "lightsalmon", "magenta", "maroon",
...
UMAP_FUN <- function(seurat_object, path, color, idents = "Celltype_plot", breaks, label, xlab = "", ylab = "", plot_label = F, res = 600, height = 8, width = 10, units = 'in'){
    ...
    theme(axis.line=element_blank(),axis.text.x=element_blank(),
          ...
          legend.title = element_text(size = 16, face = "bold", family = "Helvetica"), legend.text = element_text(size = 14))
...
# DotPlot
###-------------------####
DOTPLOT_FUN <- function(seurat_object, features, path, ident = "Annotation", width = 10, height = 5, res = 600, units = 'in') {
    ...
    plot <- DotPlot(seurat_object, features = features, group.by = ident) + scale_color_gradient2(low = "steelblue", 
                        high = "red") + 
    RotatedAxis() +
    theme(axis.text.y = element_text(size = 12, face = "bold", family = "Helvetica"),
    axis.text.x = element_text(size = 12, face = "bold", family = "Helvetica"),
    axis.title.x = element_blank(),
    axis.title.y = element_blank())
    # Save Plot
...
# Feature Plots
###-------------------####
Feature_Plot <- function(seurat_object, ADT, reduction = 'umap') {
    DefaultAssay(seurat_object) <- "ADT"
    Plot <- FeaturePlot(seurat_object, features = ADT, reduction = reduction, raster = F, min.cutoff = 'q10', max.cutoff = 'q90', cols = c("lightgrey", "darkgreen")) +
          theme(axis.line=element_blank(),axis.text.x=element_blank(),
          axis.text.y=element_blank(),axis.ticks=element_blank(),
          axis.title.x=element_blank(),
          axis.title.y=element_blank(),
...
# Scatter Plot
###-------------------####
scatter_plot <- function(DF, x = "CD4_TEM", y = "meta_fbg", xlabel = "CD4 TEM Cell Proportion (% CD4)", ylabel = "Fasting Blood Glucose (mg/dl)", dotsize = 2, nonDM = F) {
  if (nonDM == T) {
      DF <- DF %>% dplyr::filter(StudyGroup != "HIV+ diabetic")
  }
  
  plot <- DF %>% ggplot(aes_string(x = x, y = y)) +
  geom_point(size = dotsize, shape = 19) +
  xlab(xlabel) +
  ylab(ylabel) + 
  ...
  theme(axis.text = (element_text(size = 16)),
        ...
        axis.title.y = element_text(face = "bold", size = 16, color = "black", margin = margin(t = 0, r = 20, b = 0, l = 0)),
        axis.title.x = element_text(face = "bold", size = 16, color = "black", margin = margin(t = 20, r = 0, b = 0, l = 0)),
...
box_plot_fun_paper <- function(df, my_comparisons, 
                         ...
                         specify_x_order = NA, specify_x_labs = NA, size = 6,
  ...
  stat_compare_means(comparisons = my_comparisons, label.y = stat_pos, paired = is_paired, method = "wilcox.test", symnum.args = symnum.args, size = size) +
...
###############################
Cor_rho <- function(df, Measures, nonDM = T) {
    ...
    for (measure in Measures) {
        if (measure == "BMI") {
        for (i in 1:numb) {
                cor_res <- PResiduals::partial_Spearman(df[,i] | df[,measure] ~ Sex  + StudyGroup + Age, data = df, link.x = "logit", link.y = "logit")
                res <- data.frame(rho = cor_res$TS$TB$ts, pvalue = cor_res$TS$TB$pval, Cluster = colnames(df)[i], Measure = measure)
                Results <- rbind(Results, res)
            }
        } else if (measure == "Age") {
             for (i in 1:numb) {
                cor_res <- PResiduals::partial_Spearman(df[,i] | df[,measure] ~ Sex  + StudyGroup + BMI, data = df, link.x = "logit", link.y = "logit")
                res <- data.frame(rho = cor_res$TS$TB$ts, pvalue = cor_res$TS$TB$pval, Cluster = colnames(df)[i], Measure = measure)
                Results <- rbind(Results, res)
            }
        } else if (measure %in% c("meta_fbg", "meta_hba1c", "homa2_ir")) {
            for (i in 1:numb) {
                cor_res <- PResiduals::partial_Spearman(df[,i] | df[,measure] ~ Sex + BMI + Age, data = df, link.x = "logit", link.y = "logit")
                res <- data.frame(rho = cor_res$TS$TB$ts, pvalue = cor_res$TS$TB$pval, Cluster = colnames(df)[i], Measure = measure)
                Results <- rbind(Results, res)
            }
        }
...
# Heatmap Correlation
###############################
Heatmap_Glucose <- function(df, legend_breaks = c(-0.50, 0.0, 0.50), size.y = 14, size.x = 14, size.sig = 7, legend.size = 14, legend.title = 14, legend.pos = "right", plot.margin = margin(5.5, 5.5, 5.5, 5.5, "pt")) {
    df <- df %>% mutate(Measure = case_when(Measure == "meta_hba1c"~"HbA1c",
    Measure == "meta_fbg"~"FBG",
    Measure == "Age"~"Age",
    Measure == "BMI"~"BMI"))
    
    plot <- df %>% ggplot(aes(x = Cluster, y = Measure, fill = rho)) +
    geom_tile(color = "white") +
    scale_x_discrete(position = "top") +
    scale_fill_paletteer_c("ggthemes::Classic Red-Blue", direction = -1, limits = range(df$rho), breaks = legend_breaks, space = "Lab", name = "Rho") +
    theme_minimal() + 
    geom_text(aes(Cluster, Measure, label = paste0(ifelse(df$padj < 0.01, "**", ifelse(df$padj < 0.05, "*", "")))), color = "white", size = size.sig) +
    theme(axis.text.x = element_text(angle = 45, vjust = 0, size = size.x, hjust = 0, face = "bold"),
    axis.text.y = element_text(size = size.y, face = "bold"),
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
...
###############################
# Heatmap Correlation Circle
###############################
Heatmap_circle <- function(df, order = NULL, legend_breaks = c(-0.50, 0.0, 0.50), range = c(15,5), limits = range(df$rho), p_breaks = c(0.001, 0.01, 0.05, 0.1, 0.5), size.y = 14, size.x = 14, size.sig = 7, legend.size = 14, legend.title = 14, legend.pos = "right", plot.margin = margin(5.5, 5.5, 5.5, 5.5, "pt")) {
    df <- df %>% mutate(Measure = case_when(Measure == "meta_hba1c"~"HbA1c",
    Measure == "meta_fbg"~"FBG",
    Measure == "Age"~"Age",
    Measure == "BMI"~"BMI")) %>%
    mutate(Cluster = fct_relevel(Cluster, order))
    
    plot <- df %>% ggplot(aes(x = Cluster, y = Measure, fill = rho, size = padj)) + 
        geom_point(shape = 21, stroke = 0) +
        geom_hline(yintercept = seq(.5, length(unique(Cor_res$Measure)) + 0.5, 1), linewidth = .2) +
        scale_x_discrete(position = "top") + 
        scale_size(range = range, breaks = p_breaks, name = "Adjusted P value") +
        scale_fill_paletteer_c("ggthemes::Classic Red-Blue", direction = -1, limits = limits, breaks = legend_breaks, space = "Lab", name = "Rho") +
        geom_text(aes(Cluster, Measure, label = paste0(ifelse(df$padj < 0.01, "**", ifelse(df$padj < 0.05, "*", "")))), color = "white", size = size.sig) +
        theme_minimal() +
        theme(panel.grid.major = element_blank(),
            legend.position = legend.pos,
            axis.text.x = element_text(angle = 45, vjust = 0, size = size.x, hjust = 0, face = "bold"),
            axis.title.x = element_blank(),
            axis.title.y = element_blank(),
            panel.background = element_blank(),
            legend.title = element_text(size = legend.title, face = "bold"),
            legend.text = element_text(size = legend.size),
            plot.margin = plot.margin,
            axis.text.y = element_text(size = size.x, face = "bold")) +
...
###############################
Cor_rho_subset <- function(df, subset1, subset2) {
    Results <- data.frame(matrix(ncol = 4, nrow = 0))
    colnames(Results) <- c("rho", "pvalue", "Cluster1", "Cluster2")

        for (clust1 in subset1) {
                for (clust2 in subset2) {
                    cor_res <- PResiduals::partial_Spearman(df[,clust2] | df[,clust1] ~ Sex + StudyGroup + BMI + Age, data = df, link.x = "logit", link.y = "logit")
                    res <- data.frame(rho = cor_res$TS$TB$ts, pvalue = cor_res$TS$TB$pval, Cluster1 = clust1, Cluster2 = clust2)
                    Results <- rbind(Results, res)
                }
            }
    Results$padj <- p.adjust(Results$pvalue, method = "BH")
...
###############################
# Heatmap Correlation subset
###############################
Heatmap_clusters <- function(df, legend_breaks = c(-0.50, 0.0, 0.50), size.y = 14, size.x = 14, size.sig = 7, legend.size = 14, legend.title = 14, legend.pos = "right", plot.margin = margin(5.5, 5.5, 5.5, 5.5, "pt")) {
    plot <- df %>% ggplot(aes(x = Cluster1, y = Cluster2, fill = rho)) +
    geom_tile(color = "white") +
    scale_x_discrete(position = "top") +
    scale_fill_viridis(limits = range(df$rho), breaks = legend_breaks, space = "Lab", name = "Rho") +
    theme_minimal() + 
    geom_text(aes(Cluster1, Cluster2, label = paste0(ifelse(df$padj < 0.01, "**", ifelse(df$padj < 0.05, "*", "")))), color = ifelse(df$rho < 0, "white", "black"), size = size.sig) +
    theme(axis.text.x = element_text(angle = 45, vjust = 0, size = size.x, hjust = 0, face = "bold"),
    axis.text.y = element_text(size = size.y, face = "bold"),
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
...
# Heatmap Correlation Circle
###############################
Heatmap_subset <- function(df, order = NULL, legend_breaks = c(-0.50, 0.0, 0.50),limits = range(df$rho), range = c(15,5), p_breaks = c(0.001, 0.01, 0.05, 0.1, 0.5), size.y = 14, size.x = 14, size.sig = 7, legend.size = 14, legend.title = 14, legend.pos = "right", plot.margin = margin(5.5, 5.5, 5.5, 5.5, "pt")) {
    df <- df %>% 
    mutate(Cluster2 = fct_relevel(Cluster2, order))
    
    plot <- df %>% ggplot(aes(x = Cluster2, y = Cluster1, fill = rho, size = padj)) + 
        geom_point(shape = 21, stroke = 0) +
        geom_hline(yintercept = seq(.5, length(unique(Cor_res$Cluster1)) + 0.5, 1), linewidth = .2) +
        scale_x_discrete(position = "top") + 
        scale_size(range = range, breaks = p_breaks, name = "Adjusted P value") +
        scale_fill_paletteer_c("ggthemes::Classic Red-Blue", direction = -1, limits = limits, breaks = legend_breaks, space = "Lab", name = "Rho") +
        geom_text(aes(Cluster2, Cluster1, label = paste0(ifelse(df$padj < 0.01, "**", ifelse(df$padj < 0.05, "*", "")))), color = "white", size = size.sig) +
        theme_minimal() +
        theme(panel.grid.major = element_blank(),
            legend.position = legend.pos,
            axis.text.x = element_text(angle = 45, vjust = 0, size = size.x, hjust = 0, face = "bold"),
...
ORM_Model <- function(df) {
    ...
    colnames(Results) <- c("Estimate", "Lower", "Higher", "Cluster", "pvalue")
    numb <- dim(df)[2]-10
    
    for (i in 1:numb) {
        f <- rms::orm(df[,i] ~ Sex + Age + BMI + StudyGroup, data = df)
        data_orm <- data.frame(summary(f))
        Sex_odds <- data.frame(Estimate = data_orm[rownames(data_orm) == "X.Odds.Ratio.2", 4], Lower = data_orm[rownames(data_orm) == "X.Odds.Ratio.2", 6], Higher = data_orm[rownames(data_orm) == "X.Odds.Ratio.2", 7],
        Cluster = colnames(df)[i], pvalue = data.frame(anova(f))[1,3])
...
# ORM Plot
############################### 
ORM_Plot <- function(Results, title, ylabel = "", xlabel = "Odds Ratio") {
    plot <- Results %>% ggplot(aes(x = Estimate, y = Cluster)) + 
    geom_vline(xintercept = 1, linetype = "dashed") +
    geom_errorbarh(aes(xmax = Higher, xmin = Lower), size = 0.5, height = 0.2, color = "black") +
    geom_point(size = 3.5, color = "black", shape = 15) +
    ggtitle(paste("Odds Ratio Female to Male for ", title, sep = "")) +
    theme_classic() + 
    ylab(ylabel) + 
    xlab(xlabel) +
    theme(plot.title = element_text(face = "bold", size = 16, hjust = 0.5, color = "black"),
...
Heatmap_fun <- function(seurat_object, subset = F, R, metadata = data_hatim, cluster, genes, tmp_dir = paste0("/data/p_koethe_lab/Atlas_AT/HATIM_Analysis/", date, "/Dialogue/", "All"),
    ...
    # Get rid of very lowly expressed genes (likely ambient RNA contamination)
